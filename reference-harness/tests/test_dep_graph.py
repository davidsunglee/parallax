...
# consistent with the canonical claim, and exactly 116 cases carry the slice tag
...
# and to 116 when the Phase-5 versioned set-based materialize scenarios (0614/0615)
# were added (clarify transaction semantics, COR-14).
...
def test_profile_slice_tag_count() -> None:
    ...
    assert len(tagged) == 116, (
        f"expected exactly 116 cases tagged {_SLICE_TAG!r}, found {len(tagged)}: {sorted(tagged)}"
    )
