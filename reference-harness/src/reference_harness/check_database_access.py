...
them: the designated fixture must exist, and the classifier's own designated set
...
__all__ = ["Finding", "audit", "main", "seam_calls"]
...
def _resolved_target(func: ast.expr, bindings: dict[str, str]) -> str | None:
    ...
    return ".".join(parts) if head is None else ".".join([head, *parts[1:]])
...
def _declared_database_fixtures(tree: ast.Module) -> frozenset[str] | None:
    ...
    for node in tree.body:
        if not isinstance(node, ast.Assign):
            continue
        if CLASSIFIER_CONSTANT not in [t.id for t in node.targets if isinstance(t, ast.Name)]:
            continue
        value = node.value
...
def audit(tests_root: Path) -> list[Finding]:
    ...
    if not entry_module.is_file():
        ...
        allowed = _entry_point_span(
            ast.parse(entry_module.read_text(encoding="utf-8")), ENTRY_POINT_FIXTURE
        )
        if allowed is None:
            findings.append(
                Finding(
    ...
    if not classifier.is_file():
        ...
        declared = _declared_database_fixtures(ast.parse(classifier.read_text(encoding="utf-8")))
        if declared != frozenset({ENTRY_POINT_FIXTURE}):
            findings.append(
                Finding(
                    ...
                    f"{{{ENTRY_POINT_FIXTURE!r}}}, the fixture this guard permits; it reads "
                    f"{declared if declared is not None else 'a non-literal set'}",
                )
            )
...
def main(argv: list[str]) -> int:
    ...
    designated fixture; 1 — at least one does not; 2 — usage error.
    """
    if argv:
        print(
