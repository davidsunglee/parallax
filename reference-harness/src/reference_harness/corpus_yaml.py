...
# The YAML 1.2 core schema's four implicit resolvers, each listed with the first
...
_CORE_SCHEMA: tuple[tuple[str, str, tuple[str, ...]], ...] = (
...
def _construct_core_float(loader: yaml.SafeLoader, node: yaml.ScalarNode) -> float:
    ...
    (:class:`~reference_harness.portable_literal.AuthoredNumber`): which float a
    number names depends on the width the member declares, which no parser can
    see, so the seam that does see it rounds once from the literal instead of
    twice through this carrier.
    """
    text = str(loader.construct_scalar(node))
