...
- **`slice-snapshot-1`** — the **plain-value** surface: reads materialize
  ...
  round-trip observability is pinned by `m-snapshot-read`). Transient execution
  observability is lifecycle-neutral and contract-covered, but neither slice
  claims `m-execution-lifecycle` until compatibility cases carry that tag.
