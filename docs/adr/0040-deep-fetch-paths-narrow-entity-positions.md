...
source-position `appliesTo` Subtype Selection beside the ordered relationship
`segments`, whose own `narrowTo` selections constrain their target positions.
This position-and-relationship model makes inherited `Dog.owner` retain
...
A source guard restricts existing root objects and creates no relationship
view; a segment target selection populates a distinct Narrowed View. Broad and
...
broad path; and whether a segment authored a selection at all, where the view key
is derived from the authoring rather than from the resolved set. Reusing a broad
...
The canonical path-shape change lands atomically across the query schema,
...
admits a source guard that a frontend does not emit or a dialect does not
lower.
