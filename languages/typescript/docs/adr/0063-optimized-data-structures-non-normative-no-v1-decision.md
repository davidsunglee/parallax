...
A post-V1 note records the expected JS baseline so the area is not silently empty when `M8` lands: a built-in `Map` keyed by a canonical primary-key string is the idiomatic JavaScript baseline for the identity / query caches. The Java open-addressing and no-wrapper-key-allocation techniques have no compelling direct JavaScript analogue — short strings are effectively interned by the engine and V8 `Map`s are already compact, so a composite-PK string key captures the same benefit without a custom hashing strategy or open-addressing table. This is guidance for the future, not a V1 commitment; the decision is deferred with `M8` and will be made when `M8` is implemented.
