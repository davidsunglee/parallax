...
**Considered options.** Splitting the finder surface into `findOne` / `findMany` / required-result variants was rejected: `find` is the only finder operation and always returns a `ParallaxList` that may contain zero, one, or many objects, with the helper surface above answering the single-result cases. This avoids early API branching while preserving the operation-backed list semantics Parallax requires.
