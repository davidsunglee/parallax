...
Non-dependent association mutation is deferred to a future explicit `link` / `unlink` surface until the metamodel can describe association-table mappings. In v1, callers change non-dependent associations through FK updates or explicit join-entity writes.
