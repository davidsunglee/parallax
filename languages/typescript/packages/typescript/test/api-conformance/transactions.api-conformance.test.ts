 ...
 * corpus's final state. The DML is generated by the pure m-temporal-read / m-unit-work write generators
 ...
 *    each transaction stamped with a fixed clock so the Transaction-Time instants are the
...
group.skipIf(!HAS_DOCKER).each(writeProviders())("transactions suite ($label)", (dbp) => {
  ...
  it(
    ...
    async () => {
      ...
      // Milestone 1: open at 2024-01-01 (the first transaction's Transaction-Time instant).
      await f.px.transaction(async (tx) => {
        await tx.entity("Balance").create({ id: 1n, acctNum: "A", value: dec("100.00") });
      });
    ...
    async () => {
      ...
      await expect(
        f.px.transaction(async (tx) => {
          const accounts = tx.entity("Account");
          // Read account 1 first (records the observed version, m-opt-lock), then update it.
