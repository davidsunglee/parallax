...
inference and canonical-name derivation.
...
directly, so the shared lookups live HERE rather than staying duplicated.

This module is **error-neutral**: :func:`infer_neutral_type` returns the
resolved type name or ``None``, never raising — the unresolved-type message
text (and the decimal-needs-precision special case, whose fix-it snippet
differs, ``Field(...)`` vs ``VoField(...)``) is each caller's OWN
