...
1. **No-op-first.** An update whose effective change set is empty is dropped
   ...
   ``Transaction.update`` returns before buffering on the empty effective change
   set ``document_codec.classify_effective_change`` answers for a net-zero edit,
   which the keyed write ingress asks and no other module decides
