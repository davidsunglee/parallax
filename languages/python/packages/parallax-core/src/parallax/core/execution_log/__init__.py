...
would have every attribution of it resolve to whichever call was attributed
first, recording a failure against a sibling invocation's call; the contract, not
this lookup, is what forbids it. The other attributable family holds by
construction: the Affected Rows Policy raises its verdict at the rejection it is
about. A callback's own exception
