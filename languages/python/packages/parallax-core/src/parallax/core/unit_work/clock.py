...
A temporal write's **processing instant** (its ``in_z``) is never authored on a
...
conformance runner uses to pin a case's authored processing instant, and the path
...
class Clock(Protocol):
    ...
    def now(self) -> _dt.datetime:
        """The current processing instant as an aware UTC ``datetime``."""
        ...
...
class FixedClock:
    ...
    conformance path injects, pinned to a case's authored processing instant.
    """
...
def instant_literal(value: _dt.datetime) -> str:
    """Render a processing instant to the canonical neutral instant string.

    The plan carries the processing instant as context (never as an instruction
