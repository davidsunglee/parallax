"""The Entity frontend's native Unresolved Metamodel adaptation (support scope).

A class-authored model reaches Model Formation through its own Unresolved
Metamodel view rather than the descriptor scope's adapter: this builds that view
from the compiled Entity records the metaclass caches, so assembly forms from
entity-owned declarations. The adaptation is local and reference-free — every fact
that needs no cross-entity knowledge becomes its final Metadata value here, while a
relationship target, a reverse peer, and an inheritance parent stay Entity
References for the foundational resolver to advance. Nothing produced here resolves
a reference, pairs a relationship, flattens a family, or offers a lookup.
"""
...
class _EntityDeclaration:
    """One compiled Entity record seen through the Unresolved seam.

    Every member is the class's own local declaration in authoring order. The
    record it was adapted from is not retained: the view is the declaration, not a
    handle back to interchange.
    """
...
@dataclass(frozen=True, slots=True)
class _NativeMetamodel:
    """A nonempty enumeration of adapted declarations in assembly order."""

    entities: tuple[UnresolvedEntityDeclaration, ...]


def native_metamodel(metamodel: records.Metamodel) -> UnresolvedMetamodel:
    """Adapt compiled Entity records into formation's Unresolved Metamodel.

    Raises :class:`DescriptorError` when a record carries a value the model
    contract cannot represent — a nameless Sequence generation, an inheritance
    position missing the strategy or parent its role requires, or a value a model
    type refuses such as an empty namespace or a bounded length on a non-text
    Attribute. This is where a class-authored model that survives class-definition
    time but cannot form surfaces. An empty assembly is rejected here too, so
    formation never receives an empty source.
    """
    if not metamodel.entities:
        raise DescriptorError("assembly declares no entity")
    return _NativeMetamodel(tuple(_adapted(entity) for entity in metamodel.entities))
...
def _adapted(entity: records.Entity) -> UnresolvedEntityDeclaration:
    ...
    seam is where a record meets them. Their rejection is a fact about the class's
    compiled record, so it leaves here as a :class:`DescriptorError` naming the
    Entity rather than as the raw refusal.
    """
    try:
...
def _persistence(entity: records.Entity) -> PersistenceMode | None:
    ...
    inherit. An explicitly declared mode is reported as declared, so a family rule
    can still see a descendant that spelled one at all.
    """
...
def _primary_key(attribute: records.Attribute, where: str) -> AttributePrimaryKey:
    ...
    which is also how the canonical descriptor spells it.
    """
...
def _entity_reference(spelling: str) -> EntityReference:
    ...
    A bare name is relative to whichever Entity declares it, and a qualified name
    is already exact — its namespace is everything before the final dot. The
    reference keeps no owner, no raw spelling, and no fallback state.
    """
...
def _shape(
    ...
    A class spells every occurrence out in full and never names a reusable type,
    so no two occurrences share a declaration node and each therefore carries its
    own key — structurally equal shapes stay distinct declarations.
    """
