...
instance storage Pydantic's ``__dict__`` descriptor holds, and that storage is
what a pickle carries. Reaching it by name would put a class body between the
framework and its own state: ``getattr``, ``object.__getattribute__``, and
``object.__setattr__`` all resolve a name through the type, so an authored
``__dict__``, ``__getattr__``, ``__getattribute__``, or a data descriptor bound
under a slot's own name decides what the framework reads back and where its
writes land. These read and write the storage itself, so a class body can
neither hide the state a value carries, nor offer state it never carried, nor
divert what the framework attaches somewhere the framework does not look.
