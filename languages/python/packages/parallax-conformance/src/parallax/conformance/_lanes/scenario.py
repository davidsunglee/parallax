...
def compile_scenario_case(case: case_format.Case, dialect_name: str) -> tuple[list[Emission], int]:
    ...
    snapshot lane's (:func:`compile_scenario`), which the façade dispatches to."""
...
def run_scenario_case(
    ...
    grading (:func:`run_scenario`, which the façade dispatches a scenario
    carrying an action step to); a keyed unit-of-work scenario reports it
    empty. `stepGraphs` is filled on both lanes, from whichever placement of
    `expectGraph` the case authors: an `access` step's retained view there, and
    here a find step's own materialized graph (:func:`read_step_graph`) —
