...
def _set(options: object, **normalized: object) -> None:
...
class PoolOptions:
    ...
    ``min_size`` is the capacity the runtime tries to keep ready and
    ``max_size`` the ceiling it may grow to; ``max_idle`` retires capacity above
    the minimum incrementally rather than giving each connection an exact idle
    deadline, and ``max_lifetime`` retires by age with native jitter so a whole
    generation does not expire at once. Neither retirement interrupts a
    connection an operation is using.

    ``acquire_timeout`` bounds ONE caller's wait for a connection;
    ``startup_timeout`` bounds becoming ready before the handle is published;
    ``reconnect_timeout`` is how long the runtime's own background retrying
    keeps trying to restore capacity. They are three different controls and none
    of them is a retry budget for an operation. ``max_waiting`` caps how many
    callers may queue at once, with ``0`` meaning the queue is not counted.

    ``validate_on_checkout`` spends a round trip proving a connection still
    works before an operation gets it, which is what turns a connection the
    server closed while idle into a replacement rather than a failed statement.
    ...
    def __post_init__(self) -> None:
        maximum = _capacity(self.max_size)
        minimum = _integer("min_size", self.min_size)
        if not 0 <= minimum <= maximum:
            raise ValueError(f"min_size takes 0 through max_size={maximum} (got {self.min_size!r})")
        _set(self, min_size=minimum, max_size=maximum)
        _set(
            self,
            ...
            max_idle=_duration("max_idle", self.max_idle),
...
class OnDemandOptions:
    ...
    ``max_lifetime``, ``reconnect_timeout``, and ``num_workers`` still apply:
    they govern the replacement and hand-on paths rather than idle retention.
