...
afterwards, however many names the binding passed through on the way. A declared
matrix profile reaches a seam through a member rather than an importable name, so
that member is matched by name as well, on any receiver: the member is matched
rather than resolved, so a call the rule cannot type is reported rather than
trusted. That direction is deliberate — a false report is a
