...
The three RECORDING fakes — ``FakeDbPort``, ``FakeWritePort``, ``QueueDbPort`` —
are the conformance engine's own in-memory ports, each a ``ConnectsAsItself``
that answers canned rows and records the SQL it was handed. None scripts a
boundary: every transaction they run ends exactly as its body did, through
:func:`body_outcome`. What one of them varies is stated on its constructor
rather than by subclassing it, so a port that reports a zero-row shortfall, or
fails its parameterized write, or answers successive reads from a script, is
still the one recording fake.
"""
...
@runtime_checkable
class DetachableMetrics(Protocol):
    """A pool source a scripted runtime gives up at its close."""
...
class ScriptedRuntime:
    ...
    def close(self) -> None:
        ...
        The order is the contract's: detachment precedes the teardown rather
        than following it, so a sample begun after it reaches nothing.
        """
        metrics = self.adapter.metrics
        if isinstance(metrics, DetachableMetrics):
            metrics.detach()
        self.closed = True
...
# --------------------------------------------------------------------------- #
# The recording fakes: in-memory ports answering canned rows, recording SQL.  #
# --------------------------------------------------------------------------- #
