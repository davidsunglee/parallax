...
fork and its threading primitives are pinned here beside the write core they
share.
"""
...
class _BlockingExecution:
    ...
    :func:`~parallax.conformance.scenario._await_interleaved_workers` directly with
...
def test_await_interleaved_workers_unsticks_both_on_timeout_then_joins_before_raising() -> None:
    ...
    turnstile = scenario._Turnstile()  # pyright: ignore[reportPrivateUsage] - unit test drives the scenario lane's private helper directly
    ...
    with pytest.raises(EngineError, match="turnstile hand-off is missing"):
        scenario._await_interleaved_workers(  # pyright: ignore[reportPrivateUsage] - unit test drives the scenario lane's private helper directly
