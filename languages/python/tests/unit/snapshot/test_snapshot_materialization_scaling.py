...
region releases every graph it builds, so what separates the marks is what
...
neither rows, graphs, nor executions once full and is outside this size claim.
...
points of one process answer exactly. The neighbouring 64-graph item asserts
...
def _unseen_rows(layout: Layout) -> Span:
    ...
    omissions in both ranges. The region's graph is released where it is built,
