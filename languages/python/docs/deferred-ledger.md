...
- **D-93** → fixed. `m-case-format` *Resolving reads a write owes* states its premise in the terms `m-unit-work` *Write value provenance* owns — a keyed write against an existing row is licensed either by a value a managed read of the source it writes through produced or, for a row this unit of work itself inserted, by the object-keyed buffered-insert exemption, whose value keeps the NotStored provenance because no read produced it; an insert is licensed by no source value at all — and says outright that a source is not a transaction, so the one licensing read there is need not run inside the writing unit; that is what the section's own conflict-attempt paragraph already required, and the citation now names the section that exists. The same dangling *Write evidence* citation is repaired in `parallax.conformance.engine` and in `m-opt-lock-006-success`, where the rule meant was *Write Observation*.
