...
- **D-93** → fixed. `m-case-format` *Resolving reads a write owes* states round-trip accounting alone and no licensing premise of its own: what entitles a verb to write is cited to the sections that own it — `m-unit-work` *Write value provenance* for the values a value-taking verb accepts, and *Write Observation* with the evidence rules beside it for what `delete`, `terminate`, and `terminateUntil` answer to, those verbs taking no position on provenance. The section keeps the fact its own count needs — a source is not a transaction, so a resolving read need not run inside the writing unit — which is what its conflict-attempt paragraph already required. The same dangling *Write evidence* citation is repaired in `parallax.conformance.engine` and in `m-opt-lock-006-success`, where the rule meant was *Write Observation*.
...
- **D-38** → superseded. `tests/_support/mirrored_models.py` carries an explicit `UNMIRRORED` registry naming every corpus model it does not mirror, with the reason; exclusion is deliberate rather than a coverage gap.
- **D-40** → [COR-15](https://linear.app/flimflam/issue/COR-15/deferred-work-holding-list) A5. Eager `fetchall` at the adapter boundary; port-level streaming is [COR-83](https://linear.app/flimflam/issue/COR-83/stream-deep-fetch-reads-at-fixed-memory).
- **D-44** → [COR-166](https://linear.app/flimflam/issue/COR-166/python-cannot-author-an-include-path-deeper-than-two-hops-and-no). Deep-fetch depth beyond two hops.
...
- **D-51** → [COR-15](https://linear.app/flimflam/issue/COR-15/deferred-work-holding-list) A6. A defining to-one whose foreign key sits on the target side.
...
- **D-79** → withdrawn by [COR-110](https://linear.app/flimflam/issue/COR-110/logginglifecycleprovider-builds-a-field-mapping-for-every-event). The `Logger.isEnabledFor` guard the entry ruled out is taken and its prohibition reversed rather than satisfied, for three reasons the entry could not have weighed: the exact level now comes from the one match that already names a transition, so neither the approximation the ruled-out attempt made nor the detector that bounded it exists; the Logger shapes that defeat a guard defeat every library that guards on `isEnabledFor`, and Parallax accepts that standard exposure rather than holding itself to a stricter contract; and the promise that the Logger alone decides whether a record survives is given up deliberately, not preserved. `spec/python.md` states the guard and the exposure it accepts.
