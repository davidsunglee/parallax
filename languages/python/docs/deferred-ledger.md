...
- **D-93** → fixed. `m-case-format` *Resolving reads a write owes* states its premise at the altitude `m-unit-work` *Write value provenance* owns — a keyed write verb is addressed and licensed by a value a framework-managed source published, a read through that source or an insert the writing unit of work has itself buffered — and says outright that a source is not a transaction, so the licensing read need not run inside the writing unit; that is what the section's own conflict-attempt paragraph already required, and the citation now names the section that exists. The same dangling *Write evidence* citation is repaired in `parallax.conformance.engine` and in `m-opt-lock-006-success`, where the rule meant was *Write Observation*.
