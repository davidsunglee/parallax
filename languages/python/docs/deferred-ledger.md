...
- **D-71** → fixed. `document_codec.classify_effective_change` compares both sides whole and presence-preserving — with the nested-`many` exception, which has no absence to preserve and reads as the empty collection on both sides — as `core/spec/m-document-codec.md` *Managed documents and the effective change set* and `docs/adr/0003` state.
