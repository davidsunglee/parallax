...
- **D-79** → withdrawn by [COR-110](https://linear.app/flimflam/issue/COR-110/logginglifecycleprovider-builds-a-field-mapping-for-every-event). The `Logger.isEnabledFor` guard the entry ruled out is taken and its prohibition reversed rather than satisfied; `spec/python.md` states the guard and the exposure it accepts.
