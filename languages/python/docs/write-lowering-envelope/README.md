...
- The root [portfolio](portfolio.json), [summary](summary.md), and [write-lowering envelope](write-lowering.json) are the `db56a19e` capture. They are authoritative current evidence and CI input. The root write-lowering envelope is also the after operand for the rebased confound-resolution check, but it is never compared with the original Phase 2 before capture.
