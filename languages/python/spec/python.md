...
| Clean-install production smoke tests | uv-venv fixtures | `languages/python/tests/distribution/` | `uv run pytest tests/distribution/test_clean_install.py` | `python-check-dbfree` job | exercises all five §8 selective topologies in clean environments; presence of any unselected artifact fails |
