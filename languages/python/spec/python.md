...
| Claimed capability coverage | Copied verbatim from the canonical claim: the 36 `modules` below, `dialects: ["postgres"]`, the ten `caseShapes`, `caseTags.include: ["slice-snapshot-1"]`, `commands: ["describe", "compile", "run"]`, `provisioning: "self-managed"`. `modules` is the tagged-case union of the slice, **not** a dependency closure and not a packaging plan. |
| Unclaimed implementation prerequisites | None. |
