...
| Supported language/runtime versions | CPython; `requires-python >= 3.13` | each distribution's `pyproject.toml` | (local dev on any supported minor) | CI matrix 3.13 / 3.14 | support current + one prior minor; drop on upstream EOL; floor raises are reviewed spec changes |
