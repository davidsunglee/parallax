...
section and no single representation can be edited alone. Each scope is declared
by exactly one prose row, and a second row for a scope already declared fails
rather than replacing the first. In the rows, only a
