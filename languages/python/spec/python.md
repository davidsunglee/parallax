...
| Strict static typing | Pyright, strict mode, pinned version | `languages/python/pyrightconfig.json` | `uv run pyright` | `python-static` job | strict across production and tests; a `# pyright: ignore[<rule>]` or `# type: ignore[<code>]` carries its specific code and a one-line justification at the site, as `# noqa` does; strict mode makes every suppression load-bearing (removing it fails the gate), so each is a reviewed diff line rather than a spec-maintained list |
