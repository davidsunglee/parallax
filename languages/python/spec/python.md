...
| Claimed capability coverage | Copied verbatim from the canonical claim: the 35 `modules` below, `dialects: ["postgres"]`, the ten `caseShapes`, `caseTags.include: ["slice-snapshot-1"]`, `commands: ["describe", "compile", "run"]`, `provisioning: "self-managed"`. `modules` is the tagged-case union of the slice, **not** a dependency closure and not a packaging plan. |
| Unclaimed implementation prerequisites | `m-db-port` — reached via `m-unit-work`, `m-db-error`, and `m-execution-authority`; abstract port supplied by the `parallax.core.db_port` scope, concrete adapter by `parallax-postgres`; contract-covered, never case-advertised. |
...
```json
{
  ...
  "capabilities": {
    "modules": ["m-api-conformance", "m-auto-retry", "m-batch-write", "m-bitemp-write", "m-case-format", "m-conformance-adapter", "m-core", "m-db-error", "m-deep-fetch", "m-descriptor", "m-dialect", "m-document-codec", "m-edit", "m-execution-authority", "m-execution-lifecycle", "m-inheritance", "m-metamodel", "m-model-evolution", "m-model-formation", "m-navigate", "m-object-query", "m-opt-lock", "m-pk-gen", "m-predicate", "m-read-lock", "m-relationship", "m-schema-delta", "m-snapshot-read", "m-sql", "m-storage-layout", "m-temporal-read", "m-txtime-write", "m-unit-work", "m-value-object", "m-wire"],
    "dialects": ["postgres"],
...
- **Root-local identity over page-shared state.** Within one Root View, one node per
  ...
  never escapes one root: distinct result roots never coalesce their nodes, even
  inside one eager result. Their Page may nevertheless share the one Entity State
...
- **One page-owned representation.** `Materializer.read_page` reads one eager
  result or one streamed batch into aligned occurrence arrays. A Page carries
  level, concrete, logical key, raw positional witness, identity findings,
  relationship-view row, root ordinal, paging verdict, and the Entity States
  judged so far. `Materializer.roots` yields Root Views in result order. A Root
  View owns reachability, view union, classification, and representation-specific
  publication for exactly one root; it borrows its Page and retains nothing after
  ...
  Edges and roots are exact nonnegative built-in `int` occurrence indexes. A
  ...
  A milestone-set read is one database-ordered Page and one flat root tuple. It
  does not partition or import rows before publication. An undecodable temporal
  edge is ordinary in-band invalid data with `edge=None` at the row's arrival
  ordinal, in both eager and streamed checked views.
...
- **Root classification and the construction scope.** Classification runs once
  over each Root View, after state judgment and before construction, and no seam below
...
- **Shared Entity State, root-local Views.** Every consumer reads the same
  ...
  row by reference; a second Root View reaching it receives the identical state
  object but constructs a distinct node and unions only the Views reachable from
  its own root.
...
- **Projection conflict.** `SnapshotConsistencyError(RuntimeError)` is exported
...
| Clean-install production smoke tests | uv-venv fixtures | `languages/python/tests/distribution/` | `uv run pytest tests/distribution/test_clean_install.py` | `python-check-dbfree` job | exercises all seven §8 selective topologies in clean environments; presence of any unselected artifact fails |
...
- **Retained and current evidence.** The clean schema-authoritative Snapshot
  ...
  make old readings appear current. Changing an input therefore requires a new
  capture to restore current-evidence status. The `uv.lock` digest records the producing run's
